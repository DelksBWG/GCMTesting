...
public static SecretKey StringParaChave(String chaveString) {
	byte[] chaveByte = Base64.getDecoder().decode(chaveString);
		SecretKey chave = new SecretKeySpec(chaveByte, 0, chaveByte.length, "AES");
		return chave;
}
public static String ByteParaString(byte[] array) {
	return Base64.getEncoder().encodeToString(array);
}
public static byte[] StringParaByte(String byteString) {
