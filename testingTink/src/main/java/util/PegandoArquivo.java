...
public static String retornandoString(ChaveCodificar chave, GCMParameterSpec gcm, String algoritmo  //classe para alterar o File na classe arquivo utilizando a Cipher cifra
		 ) throws IOException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
    ...
    final String TRANSFORMATION = "AES/GCM/NoPadding";;
        ...
        FileInputStream inputStream = new FileInputStream("C:\\Users\\delks\\OneDrive\\Documentos\\teste.txt");
        File file = new File("C:\\Users\\delks\\OneDrive\\Documentos\\teste.txt");
        byte[] inputBytes = new byte[(int) file.length()];
        inputStream.read(inputBytes);
        BufferedReader bufferedReader = new BufferedReader(
        	      new InputStreamReader(inputStream, StandardCharsets.UTF_8));
		String arquivo = bufferedReader.lines().collect(Collectors.joining("\n"));
	Cipher cifra = Cipher.getInstance(algoritmo); //inicializar a Cipher cifra na inst�ncia do String algoritmo
